...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--collect-urls-file",
        default="",
        help="Only crawl/filter gallery URLs and write them to this text file (one URL per line)",
    )
...
def main() -> int:
    ...
    if args.collect_urls_file:
        selected_urls: list[str] = []
        for target_url in target_urls:
            if not args.required_tag and not args.required_language:
                selected_urls.append(target_url)
                continue
            try:
                page = fetch(target_url)
                target_url, page = get_gallery_page(target_url, page)
                metadata = parse_metadata(target_url, page)
                if matches_filters(metadata, args.required_tag, args.required_language):
                    selected_urls.append(target_url)
            except urllib.error.URLError as exc:
                print(f"Failed to fetch gallery {target_url}: {exc}", file=sys.stderr)

        output_path = Path(args.collect_urls_file)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text("\n".join(selected_urls) + ("\n" if selected_urls else ""), encoding="utf-8")
        print(f"Wrote {len(selected_urls)} URLs to {output_path}")
        return 0 if selected_urls else 1
